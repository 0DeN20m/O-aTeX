...
\begin{longtable}{|l|l|l|l|}
  ...
  \hline
  \texttt{\textbackslash naturalnumbers}  & \texttt{\textbackslash naturalnumbers}            & $\naturalnumbers$        & 自然数の集合          \\
  \texttt{\textbackslash integers}        & \texttt{\textbackslash integers}                  & $\integers$              & 整数の集合           \\
  \texttt{\textbackslash rationalnumbers} & \texttt{\textbackslash rationalnumbers}           & $\rationalnumbers$       & 有理数の集合          \\
  \texttt{\textbackslash realnumbers}     & \texttt{\textbackslash realnumbers}               & $\realnumbers$           & 実数の集合           \\
  \texttt{\textbackslash complexnumbers}  & \texttt{\textbackslash complexnumbers}            & $\complexnumbers$        & 複素数の集合          \\
  \texttt{\textbackslash dummy}           & \texttt{\textbackslash dummy}                     & $\displaystyle \dummy$   & ダミー変数           \\
  \hline
  ...
  \texttt{\textbackslash innerproduct}    & \texttt{\textbackslash innerproduct\{u\}\{v\}}    & $\innerproduct{u}{v}$    & 内積              \\
                                          & \texttt{\textbackslash innerproduct[V]\{u\}\{v\}} & $\innerproduct[V]{u}{v}$ & $V$における内積       \\
  \hline
  \multicolumn{4}{|l|}{\text{関数・写像関連のコマンド}}                                                                                                \\
  \hline
  \texttt{\textbackslash identitymap}     & \texttt{\textbackslash identitymap}               & $\identitymap$           & 恒等射             \\
                                          & \texttt{\textbackslash identitymap[X]}            & $\identitymap[X]$        & $X$の恒等射         \\
  \texttt{\textbackslash automorphisms}   & \texttt{\textbackslash automorphisms G}           & $\automorphisms G$       & $G$の自己同型群       \\
                                          & \texttt{\textbackslash automorphisms[N] H}        & $\automorphisms[N] H$    & $H$の$N$上の自己同型群  \\
  \texttt{\textbackslash endomorphisms}   & \texttt{\textbackslash endomorphisms G}           & $\endomorphisms G$       & $G$の自己準同型群      \\
                                          & \texttt{\textbackslash endomorphisms[N] H}        & $\endomorphisms[N] H$    & $H$の$N$上の自己準同型群 \\
  \texttt{\textbackslash homset}          & \texttt{\textbackslash homset(X, Y)}              & $\homset(X, Y)$          & Hom集合           \\
                                          & \texttt{\textbackslash homset[A](X, Y)}           & $\homset[A](X, Y)$       & $A$内のHom集合      \\
  \texttt{\textbackslash isometries}      & \texttt{\textbackslash isometries X}              & $\isometries X$          & 等長変換群           \\
                                          & \texttt{\textbackslash isometries[X]}             & $\isometries[X]$         & 等長変換群（下付き）      \\
  \hline
  \multicolumn{4}{|l|}{\text{その他の変数関連のコマンド}}                                                                                               \\
  \hline
  \texttt{\textbackslash distance}        & \texttt{\textbackslash distance\{d\}}             & $\distance{d}$           & 距離              \\
...
ベクトル空間においては，ベクトル$\vect{v}$に対してそのノルムを$\norm{\vect{v}}$と表す．
また，ベクトル$\vect{u}, \vect{v}$の内積は$\innerproduct{\vect{u}}{\vect{v}}$と表記する．
\begin{equation}
  \mathop{\mathrm{test}}\nolimits_x^y z
\end{equation}
\begin{equation}
  \mathop{\mathrm{test}_x^y}\nolimits z
\end{equation}
